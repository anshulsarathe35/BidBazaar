...
router.post("/new", async (req, res) => {
    try {
      const newProduct = new Product(req.body);
        ...
        user.itemsSold=[...user.itemsSold,id];
				user.save();
			}
...
router.post("/predict-soldDate", async (req, res) => {
  try {
      const products = await Product.find({ category: req.body.category }).sort({ price: -1 });
      let soldDate = products.length > 0 ? products[0].soldDate : null;
      
      if (!soldDate) {
          // If there is no sold date available, predict a date one week from now
          soldDate = new Date();
          soldDate.setDate(soldDate.getDate() + 7);
      }
      
      const formattedSoldDate = soldDate.toISOString().slice(0, 16);
      res.send({
          message: formattedSoldDate,
          success: true,
      });
  } catch (error) {
      res.send({
...
// GET endpoint to fetch data for the pie chart (added on 8th may for epics)
